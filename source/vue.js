...
(function(global, factory) {
  ...
  var isAndroid =
    (UA && UA.indexOf("android") > 0) || weexPlatform === "android"; //安卓
  ...
  strats.props = strats.methods = strats.inject = strats.computed = function(
    ...
    console.log("strats.props methods inject computed", ret);
    return ret;
  };
  ...
  function _traverse(val, seen) {
    //***(val)
    //***(seen.add)
    // console.log('traverse 追踪', val,seen)
    var i, keys;
    //判断是否是数组
  ...
  function extractPropsFromVNodeData(
    ...
    //***(Ctor.options)
    // console.log('extractPropsFromVNodeData   :',data,Ctor,tag)
  ...
  function normalizeArrayChildren(children, nestedIndex) {
    ...
    //返回 res 值
    console.log("normalizeArrayChildren", res);
    return res;
  }
  ...
  function lifecycleMixin(Vue) {
    ...
    Vue.prototype._update = function(vnode, hydrating) {
      ...
      //***(prevEl)
      //***(typeof prevEl)
      //***(Object.prototype.toString.call(prevEl))
      //***(vm);
  ...
  function updateChildComponent(
    ...
    var hasChildren = !!(
      ...
      ) // has old scoped slots  是否有旧的范围插槽 emptyObject 是一个空的对象
    );
  ...
  function initData(vm) {
    ...
    //获取data中的数据   判断如果是函数则
    data = vm._data =
      typeof data === "function" //如果data是函数
    ...
    while (i--) {
      ...
      var key = keys[i];
      {
        if (methods && hasOwn(methods, key)) {
          //如果数据中的 key 与事件 中的定义的key 一样 则发出警告
          warn(
      ...
      if (props && hasOwn(props, key)) {
        //如果数据中的 key 与props属性 中的定义的key 一样 则发出警告
        "development" !== "production" &&
          warn(
            'The data property "' +
              key +
  ...
  function createComponent(
    ...
    //转换v-model 并且 绑定事件
    if (isDef(data.model)) {
      //如果定义有 model 转义 model 并且绑定 v-model
    ...
    var vnode = new VNode(
      ...
      context, //vm vue实例化对象或者父组件。
      {
        ...
        tag: tag,
        children: children
      }, //当前组件 构造函数propsData属性  事件，tag标签， 子节点
      asyncFactory
    );
  ...
  function _createElement(
    ...
    if (typeof tag === "string") {
      ...
      if (config.isReservedTag(tag)) {
        ...
        //***('===Ctor===')
        //***(Ctor) //获取到 VueComponent 构造函数 sup类
  ...
  var KeepAlive = {
    ...
    render: function render() {
      ...
      if (componentOptions) {
        ...
        var key =
          vnode.key == null // 判断当前虚拟dom得key 是否为空
  ...
  function registerRef(vnode, isRemoval) {
    ...
    if (isRemoval) {
      ...
      if (Array.isArray(refs[key])) {
        //如果定义有多个同名的ref 则会定义为一个数组，删除refs 这个key 定义的数组
      ...
      if (vnode.data.refInFor) {
        //如果ref和for一起使用的时候
        if (!Array.isArray(refs[key])) {
          //refs[key] 不是数组 则变成一个数组
          refs[key] = [ref];
        } else if (refs[key].indexOf(ref) < 0) {
          //如果ref 不存在 refs的时候则添加进去
          // $flow-disable-line
          refs[key].push(ref);
        }
      } else {
  ...
  function updateAttrs(oldVnode, vnode) {
    ...
    if (isDef(attrs.__ob__)) {
      //判断attrs.__ob__ 如果定义了 就执行 从新克隆一个
   ...
   * @param {*} exp
   */
  //***(parseFilters(' ab | c | d')) //转化成 _f("d")(_f("c")(ab))
  ...
  /*
     ...
     * @returns {string}
     */
  //***(wrapFilter('abc', 'defg(hijk)')) //结果 _f("defg")(abc,hijk)
  ...
  function addHandler(
    ...
    var events;
    if (modifiers.native) {
      // 判断是有原生事件修饰符 通俗点讲：就是在父组件中给子组件绑定一个原生的事件，就将子组件变成了普通的HTML标签，不加'. native'事件是无法触  发的。
      /*
  ...
  //***(parseModel('object'))
  //***(parseModel('object[info][name]'))
  //***(parseModel('object.info.name'))
  //***(parseModel('test[key]'))
  //***(parseModel('test[test1[key]]'))
  //***(parseModel('test["a"][key]'))
  //***(parseModel('xxx.test[a[a].test1[key]]'))
  //***(parseModel('test.xxx.a["asa"][test1[key]]'))
  ...
  //***parseStyleText('width:100px;(height:200px);'))
  ...
  function getTransitionInfo(
    ...
    //***styles[transitionProp + 'Property']) //获取动画设置在哪些属性上面
  ...
  function enter(vnode, toggleDisplay) {
    ...
    while (transitionNode && transitionNode.parent) {
      //循环父层vonde 一直到顶层的 vonde
      transitionNode = transitionNode.parent;
      context = transitionNode.context;
    }

    var isAppear = !context._isMounted || !vnode.isRootInsert; //是否已经调用过Mounted 生命周期函数 //   /*是否作为跟节点插入*/
    ...
    /*
        ...
        * */
    var toClass =
      isAppear && appearToClass // 离开的过度 css 类名
        ? appearToClass
        : enterToClass;
  ...
  function parseText(
    ...
    if (lastIndex < text.length) {
      //拼接最后一个字符， 数据{{data.number}}个手机    把个手机 的字符串连接起来
  ...
  //***parseText('我叫{{name}},今年{{age}},数据{{data.number}}个手机'))
  //    //***parseText('{{name}}这个'))
  ...
  function parseHTML(
      ...
      if (
        ...
        if (textEnd === 0) {
          ...
          if (comment.test(html)) {
            ...
            if (commentEnd >= 0) {
              //如果注释标签结束标签位置大于0，则有注释内容
              //***html.substring(4, commentEnd))
              if (options.shouldKeepComment) {
                //shouldKeepComment为真时候。获取注释标签内容
          ...
          if (conditionalComment.test(html)) {
            //匹配开始为 <![ 字符串  <![endif]-->   匹配这样动态加ie浏览器的 字符串  <!--[if IE 8]><link href="ie8only.css" rel="stylesheet"><![endif]-->
            //匹配ie浏览器动态加样式结束符号
    ...
    function handleStartTag(match) {
      ...
      var unary = isUnaryTag$$1(tagName) || !!unarySlash; //函数匹配标签是否是 'area,base,br,col,embed,frame,hr,img,input,isindex,keygen, link,meta,param,source,track,wbr' //如果是/> 则为真
      ...
      if (!unary) {
        ...
        //设置结束标签
        lastTag = tagName;
        //***'== parseHTML handleStartTag stack==')
        //***stack)
      }
    ...
    function parseEndTag(
      ...
      if (pos >= 0) {
        //这里就获取到了stack堆栈的pos索引
        // Close all the open elements, up the stack 关闭所有打开的元素，向上堆栈
        //***pos)

        for (var i = stack.length - 1; i >= pos; i--) {
          if (
            "development" !== "production" && //如果stack中找不到tagName 标签的时候就输出警告日志，找不到标签
            (i > pos || !tagName) &&
            options.warn
          ) {
    ...
    parseHTML(
      template, //字符串模板
      {
        ...
        start: function start(
          ...
          var ns =
            (currentParent && currentParent.ns) || platformGetTagNamespace(tag); //判断 tag 是否是svg或者math 标签
        ...
        chars: function chars(text) {
          ...
          text =
            inPre || text.trim() //  判断标签是否是pre 如果是则返回真，则不需要去空格 //去除text空格
              ? isTextTag(currentParent) //判断标签是否是script或者是style
                ? text //直接获取文本
                : decodeHTMLCached(text) //获取 真是dom的textContent文本
              : // only preserve whitespace if its not right after a starting tag
              //只有在开始标记之后没有空格时才保留空格
              preserveWhitespace && children.length //模板编译器的选项。当使用默认的 vue-template-compiler 的时候，你可以使用这个选项来添加自定义编译器指令、模块或通过 { preserveWhitespace: false } 放弃模板标签之间的空格。
              ? " "
  ...
  function markStaticRoots(node, isInFor) {
    if (node.type === 1) {
      ...
      if (
        ...
        !(
          (node.children.length === 1 && node.children[0].type === 3) //如果只有一个子节点 //属性节点
        )
      ) {
  ...
  function isStatic(node) {
    ...
    return !!(
      ...
      (
        ...
        !node.if &&
        !node.for && // not v-if or v-for or v-else 没有 v-if 或者 v-for 或者 v-else
  ...
  function genIfConditions(
    ...
    if (!conditions.length) {
      //如果conditions 不存在 则返回一个空的虚拟dom参数
      return altEmpty || "_e()";
    }
        ...
        if (dirRE.test(name)) {
          ...
          if (value) {
            if (name === "v-for") {
              ...
              // var onRE = /^@|^v-on:/; 匹配@开头 或者是v-on: 开头
  ...
  function createCompilerCreator(
    ...
    return function createCompiler(baseOptions) {
      ...
      function compile(
        ...
        //***finalOptions)
        //***finalOptions.__proto__)
        //***finalOptions.property)
